...
dirs$rscript_name = "Task14b_Parallel_Coordinates_Soluble_2019-10-12.R.R"
...
data_summary = data_keep %>%
  group_by(AFIRsim_SCIMsim_category) %>%
  count() %>%
  arrange(desc(n))
kable(data_summary)
...
data_plot = data_keep %>%
  ...
  select(id,contains("AFIR"),contains("SCIM"), T0_tf:kon_TL_tf, dose_mpk_tf) %>%
  gather(param,param_value,-c(id, contains("AFIR"), contains("SCIM"))) %>%
  mutate(param = str_replace(param,"_tf",""))

#sort by average param value in one category to help with visualization ----
data_summ = data_plot %>%
  filter(AFIRsim_SCIMsim_category == "AFIRsim < 5%, SCIMsim > 30%") %>%
  group_by(param,AFIRsim_SCIMsim_category) %>%
...
g = g + facet_grid(SCIMsim_category~AFIRsim_category,switch = "y")
g = g + theme(axis.text.x = element_text(angle = 45, hjust = 1))
g = g + labs(x = "Parameter", y = "Parameter Value")
g = xgx_save(7,7,dirs,"Parallel_Coord_Soluble_3x3_AFIRsim_SCIMsim","")
print(g)

#2. AFIRsim vs SCIMsim : 2 colors ----
data_plot_color = data_plot %>%
  filter(AFIRsim_SCIMsim_category %in% c("AFIRsim < 5%, SCIMsim < 5%","AFIRsim < 5%, SCIMsim > 30%"))
