...
dirs$rscript_name = "Task20b_SimulationProcess_and_Explore_2e3.R"
dirs$filename_prefix= str_extract(dirs$rscript_name,"^Task\\d\\d\\w?_")

data_in = read.csv("results/Task19_2019-11-17_2e3.csv",stringsAsFactors = FALSE)
#```
...
data = data %>%
  mutate(assumption_AFIR_lt_30    = AFIR_thy < 0.30,
         ...
         assumption_drug_gg_T0    = Cavgss_thy > 5*Ttotss_thy,
         assumption_drug_gg_KssDT = Cavgss_thy > 5*Kss_DT,
