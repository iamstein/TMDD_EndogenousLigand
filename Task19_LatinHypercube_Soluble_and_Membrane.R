...
dirs$rscript_name = "Task19_LatinHypercube_Soluble_and_Membrane.R"
...
param_minmax_in = readxl::read_excel("parameters/Task19_Param_Ranges_AFIR_Tfold.xlsx")
...
for (i in 1:n_samples) { 
  ...
  if ( ((i %% n_sim)==1) & (result[[i]]$error_simulation == FALSE) ) {
    out = plot_param(result[[i]],model, plot_flag = FALSE)
