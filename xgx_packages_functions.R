...
ggdraft = function(draft.flag,x=Inf,y=Inf,label="DRAFT",color="grey",hjust=1.2,vjust=1.2,draft.font=7,fontface="bold") {
...
ggfilename = function(dirs,g=last_plot(),fontsize=8) {
...
#save plot
ggsaveplot = function(width,height,dirs,filename.main,draft.flag=FALSE,
  ...
  g = g + ggdraft(draft.flag,x=draft.x,y=draft.y,draft.font=draft.font)
  if (print.filenames.flag) g = ggfilename(dirs)
...
scale.x.log10 = function(decade.spacing=1,x1=-10,x2=10,color="grey50",...) {
...
scale.y.log10 = function(decade.spacing=1,x1=-10,x2=10,color="grey50",...) {
  if (length(intersect(c("breaks","labels"),names(list(...))))==0) {
...
if(FALSE) {
  ...
  ggplot(cars,aes(x=speed,y=dist)) + 
    geom_point() + 
    scale.x.log10(breaks=c(5,10,20)) + 
    scale.y.log10(breaks=c(1,10,100))
}
